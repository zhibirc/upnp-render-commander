...
function parseSubscriptionData ( data ) {
    ...
    if ( actualData && (parsed = actualData.match(removeWrapper)) ) {
        parsed[1] = parsed[1].replace(/&(?:[gl]|quo)t;/g, '').replace(/\r\n/g, '');
        parsed = parsed[1].split('/><'); // accept multiple tags

        parsed = parsed.map(elem => {
            return elem.split(' ').map(( elem, idx ) => {
                return idx ? elem.split('=') : elem.replace(/^[A-Z]/, char => char.toLowerCase());
            });
        });
